...
- [ ] **Parser**: `fn name(a, b) { … }` definitions and `name(arg, …)` calls;
      a program is a list of functions plus a top-level expression to evaluate.
- [ ] **Codegen — function bodies**: each function gets a prologue/epilogue
...
- [ ] **Codegen — calls**: evaluate arguments, move them into the argument
      registers, `call rel32` (`E8` + 4 bytes), result arrives in `RAX`.
- [ ] **Alignment**: keep `RSP` 16-byte aligned at every `call` (frame sizes a
      multiple of 16). Misalignment shows up as a crash inside libc, not at your
      call site — see [docs/07](../07-calling-convention.md).
- [ ] **Forward references**: record each `call`'s displacement position and the
      callee name; after all functions are emitted, patch every call with the
      real offset. (Same patch helper as milestone 2.)
- [ ] Emit all functions into **one** buffer so `rel32` distances are valid.
- [ ] **Tests**: `fn add(a,b){ a+b; } add(2,3);` → 5; a recursive factorial;
...
- [ ] `fib(30)` returns **832040** from JIT-compiled machine code.
- [ ] Recursion works to a reasonable depth without corrupting the stack.
- [ ] A function may call one defined later in the file (forward reference).
