...
- [ ] Displacements are **relative to the end of the jump instruction**; a
      helper (`patch_rel32(at, target)`) keeps this in one place.
- [ ] **Tests**: `if (1 < 2) { 10; } else { 20; }` → 10 (and the inverse → 20);
...
- [ ] Both branches of an `if`/`else` return the right value, verified by tests.
- [ ] All six comparison operators tested.
- [ ] Nested conditionals work — proof the patching is position-independent.
