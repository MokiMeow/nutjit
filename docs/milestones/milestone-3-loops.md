...
- [ ] **Parser**: `while (cond) { body }`; and assignment to an existing
...
- [ ] **Codegen**:
  ...
  - patch the exit `je`.
- [ ] Assignment reuses milestone 1's slot lookup and `mov [rbp-off], rax`.
- [ ] **Tests**: a counting loop
...
- [ ] The counting loop returns the correct sum from generated code.
- [ ] A zero-iteration loop (`while (0 < 0)`) is skipped correctly.
- [ ] A nested loop produces the right result.
